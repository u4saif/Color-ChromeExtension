...
function colorPicked(e) {
  ...
  document.getElementById(
    "rgbStringValue"
  ).value = `rgb(${rgbValue[0]},${rgbValue[1]},${rgbValue[2]})`;
  document.getElementById(
    "hslStringValue"
  ).value = `hsl(${hslValue["h"]},${hslValue["s"]}%,${hslValue["l"]}%)`;
  document.getElementById(
    "colorResult"
  ).style.background = `hsl(${hslValue["h"]},${hslValue["s"]}%,${hslValue["l"]}%)`;
}
...
function rgbColor() {
  let R =
    0 <= parseInt(document.getElementById("rvalue").value) < 256
      ? parseInt(document.getElementById("rvalue").value)
      : 0;
  let G =
    0 <= parseInt(document.getElementById("gvalue").value) <= 255
      ? parseInt(document.getElementById("gvalue").value)
      : 0;
  let B =
    0 <= parseInt(document.getElementById("bvalue").value) <= 255
      ? parseInt(document.getElementById("bvalue").value)
      : 0;
  R = 0 <= R && R < 256 ? R : 0;
  G = 0 <= G && G < 256 ? G : 0;
  B = 0 <= B && B < 256 ? B : 0;
...
function hslColor() {
  let h =
    0 <= parseInt(document.getElementById("hvalue").value) < 360
      ? parseInt(document.getElementById("hvalue").value)
      : 0;
  let s =
    0 <= parseInt(document.getElementById("svalue").value) <= 100
      ? parseInt(document.getElementById("svalue").value)
      : 0;
  let l =
    0 <= parseInt(document.getElementById("lvalue").value) <= 100
      ? parseInt(document.getElementById("lvalue").value)
      : 0;
  h = 0 <= h && h < 360 ? h : 0;
  s = 0 <= s && s < 100 ? s : 0;
  l = 0 <= l && l < 100 ? l : 0;
...
document
  ...
  ?.addEventListener("input", colorInputChanged);
document
  .getElementById("hexCopyIcon")
  ?.addEventListener("click", copyToClip.bind(event, "hexValue"));
...
document
  .getElementById("rgbStringcopy")
  ?.addEventListener("click", copyToClip.bind(event, "rgbStringValue"));
document
  .getElementById("hslStringcopy")
  ?.addEventListener("click", copyToClip.bind(event, "hslStringValue"));

document
  .getElementById("expand")
  ?.addEventListener("click", expand.bind(event, "expand"));
document
  .getElementById("collapse")
  ?.addEventListener("click", expand.bind(event, "collapse"));
document.getElementById("logo")?.addEventListener("click", showHideContactUS);
